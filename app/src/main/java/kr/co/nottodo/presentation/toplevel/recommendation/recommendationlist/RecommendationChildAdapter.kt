...
class RecommendationChildAdapter : RecyclerView.Adapter<RecommendationChildViewHolder>() {

    private val childItems = mutableListOf<RecommendationChildData>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecommendationChildViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        val binding: ItemNottodoRecommendationListBinding =
            DataBindingUtil.inflate(
                layoutInflater,
                R.layout.item_nottodo_recommendation_list, parent, false)
        return RecommendationChildViewHolder(binding)
    }
